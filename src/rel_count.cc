...
hash_map<RelEqClass, unsigned int, hash_releqc> RelCount::cmap;

RelCount::RelCount(hash_map<RelEqClass, int, hash_releqc>& map1, hash_map<RelEqClass, int, hash_releqc>& map2) : data(NULL) {
...
int RelCount::modify(RelEqClass key, short val) {
	...
	hash_map<RelEqClass, unsigned int, hash_releqc>::iterator pos = cmap.find(key);
	if (pos != cmap.end()) {
...
void RelCount::dumpIndex(std::ostream &out) {
	hash_map<RelEqClass, unsigned int, hash_releqc>::iterator i;
